...
GL.ClearColor(0.0, 0.0, 0.0, 0.0) # This Will Clear The Background 
  ...
  GLU.Perspective(45.0,Float(width)/Float(height),0.1,0.0) # Calculate The Aspect Ratio 
                                               # Of The Window
...
ReSizeGLScene = Proc.new {|width, height|
  ...
  GL.MatrixMode(GL::PROJECTION)
  GL.LoadIdentity()
  GLU.Perspective(45.0,Float(width)/Float(height),0.1,0.0)
  GL.MatrixMode(GL::MODELVIEW)
}
...
DrawGLScene = Proc.new {
  ...
  GL.Translate(1.5,0.0,-7.0)              # Move Right 3 Units, and back into 
                                          # the screen 7.0
  GL.Rotate($rquad,1.0,2.0,0.0)        # Rotate the quad on the X Axis
  ...
  $rtri=$rtri+1.0                       # Increase the rotation variable for
                                          # the Triangle
  $rquad=$rquad-1.0                  # Decrease the rotation variable for 
                                          # the Quad
